...
class Auth{
    public static function Register($username, $email, $password, $conn, &$err_message) : bool{
        if(REGISTER_ALLOW != true){
            $err_message = "Rejsetracja jest wyłączona";
            return false;
        }
    ...
    public static function isAdmin($conn) : bool{
        ...
        $user = $_SESSION['user'];
        $query = mysqli_query($conn,"SELECT rank FROM users WHERE username='$user'");
        while($row = mysqli_fetch_array($query)){
            $rank = $row['rank'];
        }
    ...
    public static function getUserID($user, $conn){
